...
namespace {
    ...
    HRESULT CompileShaderFromSource(const char *source, size_t source_size, const char *source_name, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob **ppBlobOut) {
        ...
        HRESULT hr = D3DCompile2(source, source_size, source_name, nullptr, nullptr, szEntryPoint, szShaderModel, dwShaderFlags, 0, 0, 0, 0, ppBlobOut, pErrorBlob.GetAddressOf());
...
namespace ultralight {
    ...
    void GPUDriverD3D11::CreateTexture(uint32_t texture_id, RefPtr<Bitmap> bitmap) {
        ...
        if (i != textures_.end()) {
            MessageBoxW(nullptr, L"GPUDriverD3D11::CreateTexture, texture id already exists.", L"Error", MB_OK);
            return;
        }
        ...
        if (bitmap->IsEmpty() /*|| bitmap->size() == 0*/) {
            ...
            hr = context_->device()->CreateTexture2D(&desc, &tex_data, texture_entry.texture.GetAddressOf());
            bitmap->UnlockPixels();
        }
    ...
    void GPUDriverD3D11::UpdateTexture(uint32_t texture_id, RefPtr<Bitmap> bitmap) {
        ...
        context_->context()->Map(entry.texture.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &res);
        ...
        if (res.RowPitch == bitmap->row_bytes()) {
            ...
            RefPtr<Bitmap> mapped_bitmap = Bitmap::Create(bitmap->width(), bitmap->height(), bitmap->format(), res.RowPitch, res.pData, res.RowPitch * bitmap->height(), false);
            IntRect dest_rect            = {0, 0, (int)bitmap->width(), (int)bitmap->height()};
            mapped_bitmap->DrawBitmap(dest_rect, dest_rect, bitmap, false);
        }

        context_->context()->Unmap(entry.texture.Get(), 0);
    }
    ...
    void GPUDriverD3D11::CreateGeometry(uint32_t geometry_id, const VertexBuffer &vertices, const IndexBuffer &indices) {
        ...
        hr = context_->device()->CreateBuffer(&vertex_desc, &vertex_data, geometry.vertexBuffer.GetAddressOf());
        if (FAILED(hr))
            return;
        ...
        hr = context_->device()->CreateBuffer(&index_desc, &index_data, geometry.indexBuffer.GetAddressOf());
        if (FAILED(hr))
            return;
    ...
    void GPUDriverD3D11::UpdateGeometry(uint32_t geometry_id, const VertexBuffer &vertices, const IndexBuffer &indices) {
        ...
        context_->context()->Map(entry.vertexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &res);
        memcpy(res.pData, vertices.data, vertices.size);
        context_->context()->Unmap(entry.vertexBuffer.Get(), 0);

        context_->context()->Map(entry.indexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &res);
        memcpy(res.pData, indices.data, indices.size);
        context_->context()->Unmap(entry.indexBuffer.Get(), 0);
    }
    ...
    void GPUDriverD3D11::BindTexture(uint8_t texture_unit, uint32_t texture_id) {
        ...
        if (entry.is_msaa_render_target) {
            if (entry.needs_resolve) {
                context_->context()->ResolveSubresource(entry.resolve_texture.Get(), 0, entry.texture.Get(), 0, DXGI_FORMAT_B8G8R8A8_UNORM_SRGB);
                entry.needs_resolve = false;
            }

            context_->context()->PSSetShaderResources(texture_unit, 1, entry.resolve_texture_srv.GetAddressOf());
        }
        else {
            context_->context()->PSSetShaderResources(texture_unit, 1, entry.texture_srv.GetAddressOf());
        }
    }
    ...
    void GPUDriverD3D11::BindRenderBuffer(uint32_t render_buffer_id) {
        ...
        context_->context()->PSSetShaderResources(0, 1, nullSRV);
        context_->context()->PSSetShaderResources(1, 1, nullSRV);
        context_->context()->PSSetShaderResources(2, 1, nullSRV);
        ...
        context_->context()->OMSetRenderTargets(1, &target, nullptr);
    }
    ...
    void GPUDriverD3D11::ClearRenderBuffer(uint32_t render_buffer_id) {
        ...
        context_->context()->ClearRenderTargetView(target, color);
    }

    void GPUDriverD3D11::DrawGeometry(uint32_t geometry_id, uint32_t indices_count, uint32_t indices_offset, const GPUState &state) {
...
void GPUDriverD3D11::LoadPixelShader(const char* path, ID3D11PixelShader** ppPixelShader)
{
    ...
    void GPUDriverD3D11::LoadCompiledVertexShader(unsigned char *data, unsigned int len, ID3D11VertexShader **ppVertexShader, const D3D11_INPUT_ELEMENT_DESC *pInputElementDescs, UINT NumElements, ID3D11InputLayout **ppInputLayout) {
        ...
        if (FAILED(hr)) {
            MessageBoxW(nullptr,
                L"GPUDriverD3D11::LoadCompiledVertexShader, Vertex shader could not be compiled. Check "
                L"your working directory.",
                L"Error", MB_OK);
            return;
        }
        ...
        // Create the input layout
        hr = context_->device()->CreateInputLayout(pInputElementDescs, NumElements, data, len, ppInputLayout);

        if (FAILED(hr)) {
            MessageBoxW(nullptr, L"GPUDriverD3D11::LoadCompiledVertexShader, Could not create vertex input layout.", L"Error", MB_OK);
            return;
        }
    }
    ...
    void GPUDriverD3D11::LoadCompiledPixelShader(unsigned char *data, unsigned int len, ID3D11PixelShader **ppPixelShader) {
        ...
        if (FAILED(hr)) {
            MessageBoxW(nullptr,
                L"GPUDriverD3D11::LoadCompiledPixelShader, Pixel shader could not be compiled. Check your "
                L"working directory.",
                L"Error", MB_OK);
            return;
        }
    ...
    void GPUDriverD3D11::BindShader(ShaderType shader) {
        ...
        case ShaderType::Fill: {
            ...
            context_->context()->VSSetShader(shader.first.Get(), nullptr, 0);
            context_->context()->PSSetShader(shader.second.Get(), nullptr, 0);
            break;
        }
    ...
    void GPUDriverD3D11::BindVertexLayout(VertexBufferFormat format) {
        ...
        switch (format) {
        case VertexBufferFormat::_2f_4ub_2f: context_->context()->IASetInputLayout(vertex_layout_2f_4ub_2f_.Get()); break;
        case VertexBufferFormat::_2f_4ub_2f_2f_28f: context_->context()->IASetInputLayout(vertex_layout_2f_4ub_2f_2f_28f_.Get()); break;
        };
    }
    ...
    void GPUDriverD3D11::BindGeometry(uint32_t id) {
        ...
        auto immediate_ctx = context_->context();
    ...
    ID3D11RenderTargetView *GPUDriverD3D11::GetRenderTargetView(uint32_t render_buffer_id) {
        ...
        if (i != render_targets_.end()) {
            ...
            if (j == textures_.end()) {
                MessageBoxW(nullptr, L"GPUDriverD3D11::BindRenderBuffer, render target texture id doesn't exist.", L"Error", MB_OK);
                return nullptr;
            }
    ...
    ComPtr<ID3D11SamplerState> GPUDriverD3D11::GetSamplerState() {
        ...
        if (FAILED(hr))
            MessageBoxW(nullptr, L"GPUDriverD3D11::GetSamplerState, unable to create sampler state.", L"Error", MB_OK);
    ...
    ComPtr<ID3D11Buffer> GPUDriverD3D11::GetConstantBuffer() {
        ...
        if (FAILED(hr))
            MessageBoxW(nullptr, L"GPUDriverD3D11::GetConstantBuffer, unable to create constant buffer.", L"Error", MB_OK);
    ...
    void GPUDriverD3D11::SetViewport(uint32_t width, uint32_t height) {
        ...
        vp.TopLeftX = 0;
        vp.TopLeftY = 0;
        context_->context()->RSSetViewports(1, &vp);
    }
    ...
    void GPUDriverD3D11::UpdateConstantBuffer(const GPUState &state) {
        ...
        for (size_t i = 0; i < state.clip_size; ++i) uniforms.Clip[i] = DirectX::XMMATRIX(state.clip[i].data);

        D3D11_MAPPED_SUBRESOURCE res;
        context_->context()->Map(buffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &res);
        memcpy(res.pData, &uniforms, sizeof(Uniforms));
        context_->context()->Unmap(buffer.Get(), 0);
    }
