...
namespace Framework::Launcher::RGL {
    ...
    /**
     * RGL titles start in a stub of their own (a .rkstr section) that loads MTLX.dll, spins on its
     * X export until the launcher answers, then jumps to the game's real entry point. Nothing
     * in-process can answer that handshake, and the stub retries forever rather than failing.
     */
    enum class EntryStubStatus {
        NOT_PRESENT,
        RESOLVED,
        UNSUPPORTED // carries a stub section this build cannot decode
    };

    struct EntryStub {
        EntryStubStatus status = EntryStubStatus::NOT_PRESENT;
        uintptr_t entryPoint   = 0; // absolute, only set when RESOLVED
    };
