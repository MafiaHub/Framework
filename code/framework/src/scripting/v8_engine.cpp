...
namespace {
    ...
    bool ReadFileContents(const std::string &filepath, std::string &outContent) {
        ...
        if (size < 0 || size > kMaxModuleFileSize) {
            Framework::Logging::GetLogger(FRAMEWORK_INNER_SCRIPTING)->warn(
    ...
    void PromiseRejectCallback(v8::PromiseRejectMessage message) {
        if (message.GetEvent() == v8::kPromiseRejectWithNoHandler) {
            ...
            if (!value.IsEmpty()) {
                ...
                if (*str) {
                    Framework::Logging::GetLogger(FRAMEWORK_INNER_SCRIPTING)->error("Unhandled promise rejection: {}", *str);
                }
            } else {
                Framework::Logging::GetLogger(FRAMEWORK_INNER_SCRIPTING)->error("Unhandled promise rejection (no value)");
            }
        }
    }

    using RequireData = Framework::Scripting::V8Engine::RequireData;
    ...
    void RequireCallback(const v8::FunctionCallbackInfo<v8::Value> &args) {
        ...
        auto *data = static_cast<RequireData *>(external->Value());
    ...
    using TimerCallbackData = Framework::Scripting::V8Engine::TimerCallbackData;

    // V8 callback: setTimeout / setInterval
    void SetTimerCallback(const v8::FunctionCallbackInfo<v8::Value> &args) {
        v8::Isolate *isolate = args.GetIsolate();
        v8::HandleScope handleScope(isolate);
        ...
        v8::Local<v8::External> external = v8::Local<v8::External>::Cast(args.Data());
        auto *data = static_cast<TimerCallbackData *>(external->Value());
        ...
        // Get the timer ID from engine (access via engine pointer)
        // We need to create the timer entry — this is done by the engine
        // For now, we build it here and let the engine store it
        auto entry = std::make_unique<Framework::Scripting::V8Engine::TimerEntry>();
        // ID will be assigned by the engine
    ...
    void ClearTimerCallback(const v8::FunctionCallbackInfo<v8::Value> &args) {
        v8::Isolate *isolate = args.GetIsolate();
        ...
        v8::Local<v8::External> external = v8::Local<v8::External>::Cast(args.Data());
        auto *engine = static_cast<Framework::Scripting::V8Engine *>(external->Value());
    ...
    void QueueMicrotaskCallback(const v8::FunctionCallbackInfo<v8::Value> &args) {
        v8::Isolate *isolate = args.GetIsolate();
