...
namespace Framework::Scripting::JS {
    ...
    /**
     * Configuration for the JSResourceManager.
     */
    struct JSResourceManagerConfig {
     ...
     */
    struct JSResourceOperationResult {
        bool success = false;
        std::string error;
        std::vector<std::string> affectedResources;

        static JSResourceOperationResult Success(const std::vector<std::string> &affected = {}) {
            JSResourceOperationResult result;
            result.success = true;
            result.affectedResources = affected;
            return result;
        }

        static JSResourceOperationResult Failure(const std::string &error) {
            JSResourceOperationResult result;
     ...
     */
    using JSResourceEventCallback = std::function<void(const std::string &resourceName)>;
    using JSResourceErrorCallback = std::function<void(const std::string &resourceName, const std::string &error)>;
    using JSResourceStateCallback = std::function<void(const std::string &resourceName, ResourceState oldState, ResourceState newState)>;
    ...
    class JSResourceManager final {
      public:
        explicit JSResourceManager(Engine *jsEngine, const JSResourceManagerConfig &config = {});
        ~JSResourceManager();

        // Non-copyable
        JSResourceManager(const JSResourceManager &) = delete;
        JSResourceManager &operator=(const JSResourceManager &) = delete;

        // Configuration
        const JSResourceManagerConfig &GetConfig() const;
        void SetConfig(const JSResourceManagerConfig &config);
         ...
         */
        JSResourceOperationResult StartResource(const std::string &name);
         ...
         * @return Result with list of stopped resources
         */
        JSResourceOperationResult StopResource(const std::string &name);
         ...
         * @return Result indicating success or failure
         */
        JSResourceOperationResult RestartResource(const std::string &name);
         ...
         * @return Result indicating success or failure
         */
        JSResourceOperationResult ReloadResource(const std::string &name);
         ...
         */
        const JSResource *GetResource(const std::string &name) const;
         ...
         */
        void SetOnResourceStarted(JSResourceEventCallback callback);

        /**
         * Set callback for when a resource stops.
         */
        void SetOnResourceStopped(JSResourceEventCallback callback);

        /**
         * Set callback for when a resource encounters an error.
         */
        void SetOnResourceError(JSResourceErrorCallback callback);

        /**
         * Set callback for resource state changes.
         */
        void SetOnResourceStateChanged(JSResourceStateCallback callback);
      ...
      private:
        ...
        JSResource *GetResourceMutable(const std::string &name);
        ...
        bool ExecuteResourceScript(JSResource &resource, std::string &outError);
        ...
        // Configuration
        JSResourceManagerConfig _config;

        // JS engine (not owned)
        Engine *_jsEngine = nullptr;

        // Resource registry
        std::map<std::string, std::unique_ptr<JSResource>> _resources;
        ...
        // Event callbacks
        JSResourceEventCallback _onResourceStarted;
        JSResourceEventCallback _onResourceStopped;
        JSResourceErrorCallback _onResourceError;
        JSResourceStateCallback _onResourceStateChanged;
