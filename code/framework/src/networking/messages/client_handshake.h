...
namespace Framework::Networking::Messages {
    class ClientHandshake final : public IMessage {
      ...
      public:
        ...
        void FromParameters(const std::string &playerName, const std::string &playerSteamId,
                            const std::string &playerDiscordId, const std::string &clientVersion,
                            const std::string &mpClientVersion, const std::string &mpClientGame) {
        ...
        bool Valid() const override {
            return _playerName.GetLength() > 0 &&
                   (_playerSteamId.GetLength() > 0 || _playerDiscordId.GetLength() > 0) &&
                   _clientVersion.GetLength() > 0 && _mpClientVersion.GetLength() > 0 && _mpClientGame.GetLength() > 0;
        }
